...
\headerbox{Summary}{name=abstract,column=0,span=2,row=0}{ \large In
  cluster outskirts, accretion driven processes lead to an evolution
  of temperature profiles scaled with respect to $R_{500c}$, breaking
  the self-similar evolution found at intermediate radii. With
...
%----------------------------------------------------%
\begin{center}
    \begin{tikzonimage}[width=.85\linewidth,trim=18 18 16 17, clip]{figures/f4.pdf}
      ...
      \node at ([text, text width=3cm, align=center] 0.45,.33) {\color{red}\bf\large $\sim30\%$ increase in};
      \node at ([text, text width=3cm, align=center] 0.45,.28) {\color{red}\bf\large clumpier structure};
      ...
      \node at ([text, text width=3cm, align=center] 0.95,.475) {\color{pink}\bf\normalsize 1$\sigma$};
      \end{tikzonimage}
\end{center}
\begin{center}
\captionof{figure}{\large\alignleft Top: density profile ratio
  of ``all'' to ``bulk'' gas.  The shaded region is the $1\sigma$
  spread about the mean of the $z=0$ profile.  ``Bulk'' is defined as
  gas with density below $2\sigma$ from the median of the density PDF
  in each radial bin.  The contribution of clumpier structures to the
  density profile in cluster outskirts increases with redshift. These
  relatively clumpy structures correspond to accreting subhalos and
  penetrating filaments. Bottom: Ratio of each profile to $z=0$ profile.}
\end{center}
\end{multicols}
}
%----------------------------------------------------------------------------------------
...
\begin{center}
      ...
      \node at ([text, text width=3cm, align=center] 0.75,.53) {\color{pink}\bf\small 1$\sigma$};        
      ...
      \end{tikzonimage}
\vspace{-12pt} \captionof{figure}{\large\alignleft $T_{mw}$
  (mass-weighted), $T_{nt}\equiv \frac{\mu m_p}{3k_b} \langle
  v^2_{\text{gas}}\rangle_{\rm mw}$ (non-thermal), and
  $T_{tot}\equiv{T}_{mw}+T_{nt}$ (total temperature), each scaled to
  $T_{500c}$: $\tilde{T}_{mw,nt,tot}\equiv{T}_{mw,nt,tot}/T_{500c}$.  $T_{tot}$ self-similarly evolves with the critical
  density; $\Delta={500c}$ scales out its
  evolution.\cite{avestruz_etal16} The potential well evolution
  exhibits the same self-similar scaling.\cite{lau_etal15} The
  evolution of $\tilde{T}_{mw}$ mostly comes from
  stirred gas.}
\end{center}
%-------------------------------%
  \begin{center}
    ...
    \end{tikzonimage}
\vspace{-20pt} \captionof{figure}{\large\alignleft Ratio of average
  temperature profiles with all of the gas to the ``bulk'' component
  (defined in Fig.~1).  At ${R}\approx1.5{R}_{500c}$, substructure
  causes no more than a $10\%$ evolution in our sample from $0<z<0.7$.
  Here, clumped evolution is comparable between $T_{mw}$
  (mass-weighted) and $T_{sp}$ (spectroscopic temperature, weighted
  by, $w=\rho_g^2\Lambda_{eff}(T,Z=0.3Z\odot)$).}
  \end{center}
\end{multicols}
}
%----------------------------------------------------------------------------------------
...
\headerbox{Scale out accretion with $\Delta=200m$}{name=R200m,column=2,span=2,above=bottom,below=stirring,bottomaligned=evolvingaccretion}{
  \begin{multicols}{2}
  \begin{center}
    \begin{tikzonimage}[width=0.88\linewidth, trim=0 3 0 5, clip]{figures/MeanTscaling.png}
      ...
      \node at ([text, text width=3cm, align=center] 0.35,.825) {\color{pink}\bf\normalsize 1$\sigma$};        
